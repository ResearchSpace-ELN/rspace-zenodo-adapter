...
class ZenodoRSpaceRepositoryTest {
    ...
    public void setUp() throws MalformedURLException {
        ...
        RepositoryConfig config = new RepositoryConfig(new URL("https://sandbox.zenodo.org/api"), "<api-token>", null, "app.zenodo");
    ...
    void testSubmitDeposit() throws IOException {
        when(zenodoClient.createDeposition(any(ZenodoSubmission.class))).thenReturn(getZenodoDeposition());
        when(zenodoClient.depositFile(any(ZenodoDeposition.class), any(String.class), any(File.class))).thenReturn(getZenodoFile());
    ...
    private ZenodoFile getZenodoFile() throws IOException {
        ...
        return mapper.readValue(IOUtils.resourceToString("/fileDepositResponse.json", StandardCharsets.UTF_8), ZenodoFile.class);
    }

    private ZenodoDeposition getZenodoDeposition() throws IOException {
        mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        return mapper.readValue(IOUtils.resourceToString("/depositionCreationResponse.json", StandardCharsets.UTF_8), ZenodoDeposition.class);
    }
